function [ allottedPats ] = DistrubutePatients_AP( N, failedNode, maxSteps, averages )
...
if nnz(isemember(find(~(any(A) | any(A'))), failedNode))
...
A(:,failedNode) = 0;
%Row normalise A so what whe now see is the probability of selecting a
%given outgoing link
        ...
        for i = 1:numberDocs
            ...
            if logical(intake(i))
                ...
                if numel(presentPats) > intake(i)
                    ...
                    kept = randsample(presentPats, int64(intake(i)), false);
            ...
            looking(j, time) = looging(j, time-1)-intake;
        end
...
allottedPats = ones(size(mean(looking)))*sum(N.node(:).mu)-mean(looking);
