load('doc_dbSA.mat')
vars = doctor_dbSA.Properties.VariableNames;
statarray = doctor_dbSA(1,:);
for i = 1:121
    for j  = 1:numel(vars)
        statarray.(j)(i,1) = median(table2array(doctor_dbSA(doctor_dbSA.distnum == i, j)));
    end
    docnum(i) = sum(doctor_dbSA.distnum == i);
end
statarray.DataID = [];
statarray.peterID = [];
statarray.docnum = docnum';
statarray = statarray(:,[17 1:16]);
load('mods.mat')
% for i = 1:24
...
% end
statarray = statarray(:,[4 1:3 5:end]);
...
h = figure;set(h);
for i = 1:numel(Preds)
    models(i).lm=fitlm(RegressionData, ['log_mean_disp ~' Preds{i}]);
    plot(models(i).lm);
    h1=get(gca,'title');
...
modelscale = struct;
for i = 4:numel(Preds)
    modelscale(i).lm = fitlm(RegressionData, ['log_mean_disp ~ ' Preds{i} scale]);
end

net = [scale ' + ' Preds{4} ' + ' Preds{5} ' + ' Preds{6} ' + ' Preds{7} ' + ' Preds{8}  '+ ' Preds{9} ' + ' Preds{10} ' + ' Preds{11} ' + ' Preds{12} ' + ' Preds{13}];
modelnet = struct;
for i = 14:numel(Preds)
    modelnet(i).lm = fitlm(RegressionData, ['log_mean_disp ~ ' Preds{i} net]);
end
...
h = figure;set(h);
for i = 1:numel(Preds)
    models_s(i).lm=fitlm(RegressionData, ['log_mean_losts ~' Preds{i}]);
    plot(models_s(i).lm);
    h1=get(gca,'title');
...
modelscale_s = struct;
for i = 4:numel(Preds)
    modelscale_s(i).lm = fitlm(RegressionData, ['log_mean_losts ~ ' Preds{i} scale]);
end

modelnet_s = struct;
for i = 14:numel(Preds)
    modelnet_s(i).lm = fitlm(RegressionData, ['log_mean_losts ~ ' Preds{i} scale net]);
end
